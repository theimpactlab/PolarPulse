...
// Reverse lookup: behavior -> category
const BEHAVIOR_TO_CATEGORY: Record<string, string> = {};
Object.entries(BEHAVIOR_CATEGORIES).forEach(([cat, behaviors]) => {
  behaviors.forEach((b) => { BEHAVIOR_TO_CATEGORY[b] = cat; });
});
  ...
  const saveBehaviors = useCallback(async () => {
    ...
    try {
      ...
      const upsertData = allBehaviors.map((behavior) => ({
        user_id: userId,
        date: date,
        category: BEHAVIOR_TO_CATEGORY[behavior] ?? "Other",
        behavior: behavior,
