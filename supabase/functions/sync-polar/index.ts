...
function isJsonContentType(res: Response): boolean // Supabase Edge Function: sync-polar
// Deploy with: supabase functions deploy sync-polar
//
// Deno runtime
...
async function refreshTokenIfNeeded(supabase: any, userId: string, token: any): Promise<string> {
  ...
  if (!token?.refresh_token) {
    throw new Error("Missing refresh_token for Polar");
  }
...
async function syncExercises(
  ...
  const list: unknown[] = exercises?.exercises ?? [];
  let synced = 0;
...
async function syncSleep(
  ...
  const list: unknown[] = sleepData?.sleeps ?? [];
  let synced = 0;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (bodyUserId) {
      ...
      // Optional fallback: sync all connected users (cron style)
    ...
    if (users.length === 0) {
      return jsonResponse({ error: "No users found to sync" }, 400);
    }
    ...
    for (const user of users) {
      ...
      try {
        ...
        const sleepSynced = await syncSleep(supabase, user.id, Number(polarUserId), accessToken);
